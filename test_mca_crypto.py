...
# CryptoJS MD5 outputs binary or hex word array
# In CryptoJS: CryptoJS.MD5(str) -> WordArray
# When passed to PBKDF2 as salt, CryptoJS converts WordArray to words/bytes
salt_bytes = hashlib.md5("fc74a45dsalt".encode('utf-8')).digest()
iv_bytes = hashlib.md5("c29aab06iv".encode('utf-8')).digest()

def derive_key():
    # CryptoJS PBKDF2 defaults to SHA1 unless specified, keySize: 128/32 = 4 words = 16 bytes, iterations: 100
    kdf = PBKDF2HMAC(
...
# Test roundtrip
test_msg = "USERID=anonymous"
enc = mca_encrypt(test_msg)
dec = mca_decrypt(enc)
print("Roundtrip test:", test_msg == dec, "Encrypted:", enc)

# Now test against live IEPF endpoint
...
try:
    r = c_requests.get(url, params=params, headers=headers, impersonate="chrome120", verify=False)
    print("Live IEPF getIepfCompNameDetails status:", r.status_code)
